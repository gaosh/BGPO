...
class CosLR:
    def __init__(self, lr, T_max, min_lr=0,):
        self.init_lr = lr
        self.T_max = T_max
        self.min_lr = min_lr
        self.steps=0
    def step(self):
        lr = self.min_lr + 0.5*(self.init_lr-self.min_lr)*( 1+math.cos(math.pi * (self.steps/self.T_max)) )
        self.steps+=1
        return lr
...
class LinearLR:
    def __init__(self, lr, T_max, min_lr=0,):
        ...
        self.steps=0
    def step(self):
        lr = self.min_lr + (1 - self.steps/self.T_max)* (self.init_lr-self.min_lr)
        self.steps+=1
        return lr
